...
export const Carousel360: React.FC<Carousel360Props> = ({
  ...
  const imageList = propImages && propImages.length > 0 ? propImages : images;
  ...
  const centerImage = imageList[centerIndex];
  ...
  useEffect(() => {
    setLoadedThumbs(imageList.map(() => false));
  }, [imageList]);
  ...
  return (
    <div className={`relative w-full flex flex-col items-center justify-center select-none py-6 sm:py-10 ${className ?? ""}`}>
      <div
        ref={containerRef}
        ...
        >
          {imageList.map((item, index) => {
            ...
            return (
              ...
              >
                <motion.div
                  className="relative rounded-lg sm:rounded-xl overflow-hidden shadow-[0_6px_20px_rgba(0,0,0,0.15)] cursor-pointer"
                  style={{ transformStyle: "preserve-3d" }}
                  animate={{
                    rotateY: -targetAngle,
                  ...
                  transition={springTransition}
                  onClick={() => {
                    const diff = ((index - centerIndex + numImages) % numImages);
                    if (diff !== 0) {
